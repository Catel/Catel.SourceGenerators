namespace Catel.SourceGenerators.Tests.XamlConstructors
{
    ...
    {
        [Test]
        public async Task Generates_Two_Ctors_For_ViewModel_With_Nullable_Injected_Model()
        {
            var driver = BuildViewModelWithNullableInjectedModelDriver();

            await Verifier.Verify(driver)
                .ScrubAssemblyVersion();
        }

        private GeneratorDriver BuildViewModelWithNullableInjectedModelDriver()
        {
            var viewModelSource = @"
#nullable enable
