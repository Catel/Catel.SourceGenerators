...
{
    [Test]
    public async Task Generates_GetService_For_Behavior_With_Existing_Ctor()
    {
        var driver = BuildBehaviorWithExistingCtorDriver();

        await Verifier.Verify(driver)
            .ScrubAssemblyVersion();
    }

    private GeneratorDriver BuildBehaviorWithExistingCtorDriver()
    {
        var behaviorSource = @"
